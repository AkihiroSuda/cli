...
func runDialStdio(dockerCli command.Cli) error {
	...
	go func() {
		stdin2conn <- copier(connHalfCloser, &halfReadCloserWrapper{os.Stdin})
	}()
	go func() {
		conn2stdout <- copier(&halfWriteCloserWrapper{os.Stdout}, connHalfCloser)
	}()
	select {
	case err = <-stdin2conn:
		if err != nil {
...
func copier(to halfWriteCloser, from halfReadCloser) error {
	defer func() {
		if err := from.CloseRead(); err != nil {
			logrus.Error(errors.Wrapf(err, "error while CloseRead (from=%+v)", from))
		}
		if err := to.CloseWrite(); err != nil {
			logrus.Error(errors.Wrapf(err, "error while CloseWrite (to=%+v)", to))
		}
	}()
