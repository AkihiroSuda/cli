...
func GetConnectionHelper(daemonURL string) (*ConnectionHelper, error) {
	...
	case "ssh":
		sshCmd, err := ssh.NewSSHCmd(daemonURL)
		if err != nil {
			return nil, err
		}
		return &ConnectionHelper{
			Dialer: func(ctx context.Context, network, addr string) (net.Conn, error) {
				return newCommandConn(ctx, sshCmd.Cmd, sshCmd.Args...)
			},
			Host: "http://docker",
		}, nil
	}
...
func (c *commandConn) onEOF(eof error) error {
	...
	c.stderrMu.Unlock()
	return errors.Errorf("command %s has exited with %v, stderr=%q", c.cmd.Path, werr, stderr)
}
